...
def update_total_rides(value, slider_value):
    return ("Total # of rides: {:,d}"
            .format(len(get_dataframe(None, value, slider_value))))
...
def update_total_rides_selection(value, slider_value, selection):
    if(selection is None or len(selection) is 0):
        return ""
    totalInSelction = 0
    df = get_dataframe(None, value, slider_value)
    for x in selection:
        totalInSelction += len(df[df.index.hour == int(x)])
...
def get_histogram(value, slider_value, selection):
    xVal = []
    yVal = []
    xSelected = []
    ...
    for i in range(0, 24, 1):
        if i in xSelected and len(xSelected) < 24:
            colorVal[i] = ('#FFFFFF')
        xVal.append(i)
        yVal.append(len(df[df.index.hour == i]))
...
def update_histogram(value, slider_value, selection):
    ...
    [xVal, yVal, xSelected, colorVal] = get_histogram(value, slider_value,
                                                      selection)
...
def filterDF(dataframe):
    dataframe.index = pd.to_datetime(dataframe["Date/Time"],
                                     format="%Y-%m-%d %H:%M:%S")
    dataframe.drop("Date/Time", 1, inplace=True)
    return dataframe

def get_dataframe(selectedData, value, slider_value):
    conn = sqlite3.connect("uber_rides.db", check_same_thread=False)
    dateLower = "2014-"+str(getIndex(value)).zfill(2)+"-"+str(slider_value).zfill(2)
    dateUpper = "2014-"+str(getIndex(value)).zfill(2)+"-"+str(slider_value+1).zfill(2)
    # print(dateLower)
    # print(dateUpper)
    if(selectedData is None or len(selectedData) is 0):
        query = '''SELECT Lat, Lon, "Date/Time" from uber_rides WHERE "Date/Time" BETWEEN "'''+dateLower+'''" AND "''' + dateUpper + '''";'''
        df = pd.read_sql_query(query, conn)
        conn.close()
        return filterDF(df)
    elif(int(selectedData[len(selectedData)-1])-int(selectedData[0])+2 == len(selectedData)+1 and len(selectedData) > 2):
        newDateLower = dateLower+" " + str(int(selectedData[0])).zfill(2)
        newDateUpper = dateLower+" " + str(int(selectedData[len(selectedData)-1])).zfill(2)
        query = '''SELECT Lat, Lon, "Date/Time" from uber_rides WHERE "Date/Time" BETWEEN "'''+newDateLower+'''" AND "''' + newDateUpper + '''";'''
        df = pd.read_sql_query(query, conn)
        conn.close()
        return filterDF(df)
    else:
        ...
        for point in selectedData:
            newDateLower = dateLower + " " + str(int(point)).zfill(2)
            newDateUpper = dateLower + " " + str(int(point)+1).zfill(2)
            query = '''SELECT Lat, Lon, "Date/Time" from uber_rides WHERE "Date/Time" BETWEEN "'''+newDateLower+'''" AND "''' + newDateUpper + '''";'''
            df = df.append(pd.read_sql_query(query, conn), ignore_index=True)
        conn.close()
        return filterDF(df)
...
def update_graph(value, slider_value, selectedData, prevLayout, mapControls):
    ...
    df = get_dataframe(selectedData, value, slider_value)
    return go.Figure(
        data=Data([
            Scattermapbox(
                lat=df['Lat'],
                ...
                text=df.index.hour,
                marker=Marker(
                    color=np.append(np.insert(df.index.hour, 0, 0), 23),
